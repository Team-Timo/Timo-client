...
PREFIX는 작업 성격에 따라 위 목록 외에도 자유롭게 지정할 수 있다.
...
| 스코프   | 기준                                                         |
| -------- | ------------------------------------------------------------ |
| `web`    | `apps/timo-web/**` 관련 작업                                 |
| `ui`     | `packages/ui/**` 관련 작업                                   |
| `root`   | 루트 설정 파일 관련 작업                                     |
| `config` | `packages/eslint-config/**`, `packages/typescript-config/**` |

- 설명은 **영어** kebab-case로 변환 (한국어 그대로 금지)
- 최대 50자 이내로 축약
