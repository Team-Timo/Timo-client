...
- 제목: `<타입>(<스코프>): <한국어 제목> (#이슈번호)` 형식 엄수
  - 좋은 예: `feat(web): 소셜 로그인 컴포넌트 추가 (#5)`
  - 나쁜 예: `feat: [web] 소셜 로그인 추가 (#5)` — 대괄호 스코프 금지
- 본문: 반드시 `~했습니다` 체로 종결
  - 좋은 예: `- 카카오 OAuth 연동 로직을 추가했습니다`
  - 나쁜 예: `- 카카오 OAuth 연동 로직 추가` — 명사형 금지
- 이슈 번호를 모르면 사용자에게 확인한다
